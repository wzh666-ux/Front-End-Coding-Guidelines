...
### [2.1](#references--prefer-const) 所有的赋值都用 `const`，避免使用 `var`。
eslint: [`prefer-const`](http://eslint.org/docs/rules/prefer-const), [`no-const-assign`](http://eslint.org/docs/rules/no-const-assign)
...
### [2.2](#references--disallow-var) 对参数重新赋值，使用 `let`，而不是 `var`。
...
### [2.3](#references--block-scope) `let` 和 `const` 都是块级作用域， 而 `var` 是函数级作用域
...
### [3.6](#objects--quoted-props) 只对那些无效的标示使用引号 `''`。
...
### [3.7](#objects--prototype-builtins) 不要直接调用 `Object.prototype`上的方法，
...
### [3.8](#objects--rest-spread) 对象浅拷贝时，更推荐使用扩展运算符（即 `...` 运算符），而不是 [`Object.assign`](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/Object/assign)。
eslint: [`prefer-object-spread`](https://eslint.org/docs/rules/prefer-object-spread)
  > 获取对象指定的几个属性时，用对象的 rest 解构运算符（即 `...` 运算符）更好。
...
### [4.7](#arrays--callback-return) 在数组方法的回调函数中使用 return 语句。如果函数体由一条返回一个表达式的语句组成，并且这个表达式没有副作用， 这个时候可以忽略 return，
eslint: [`array-callback-return`](http://eslint.org/docs/rules/array-callback-return)
> 详见 [8.2](#arrows--implicit-return)。
...
### [5.1](#destructuring--object) 用对象的解构赋值来获取和使用对象某个或多个属性值。
...
### [6.3](#es6-template-literals) 当需要动态生成字符串时，使用模板字符串而不是字符串拼接。
...
### [6.4](#strings--eval) 永远不要使用 `eval()`，该方法有太多漏洞。
eslint: [`no-eval`](https://eslint.org/docs/rules/no-eval)

<a name="6.5"></a>
<a name="strings--escaping"></a>

### [6.5](#strings--escaping) 不要使用不必要的转义字符。
...
### [7.1](#functions--declarations) 使用命名函数表达式而不是函数声明。
...
### [7.2](#functions--iife) 把立即执行函数包裹在圆括号里。
...
### [7.3](#functions--in-blocks) 不要在非函数块（`if`、`while` 等）内声明函数。
eslint: [`no-loop-func`](http://eslint.org/docs/rules/no-loop-func)
> 把这个函数分配给一个变量。浏览器会允许你这样做，但不同浏览器的解析方式不同，这是一个坏消息。
...
### [7.5](#functions--arguments-shadow) 不要用 `arguments` 命名参数。
...
### [7.6](#es6-rest) 不要使用 `arguments`，用收集参数语法 `...` 代替。
...
### [7.10](#functions--constructor) 不要用函数构造器创建函数。
...
### [7.11](#functions--signature-spacing) 函数定义部分要有空格。
...
### [7.12](#functions--mutate-params) 不要修改参数. 
...
### [7.13](#functions--reassign-params) 不要对参数重新赋值。
eslint: [`no-param-reassign`](http://eslint.org/docs/rules/no-param-reassign)
...
### [7.14](#functions--spread-vs-apply) 使用拓展运算符调用多参数的函数。
...
### [8.1](#arrows--use-them) 当你一定要用函数表达式（在回调函数里）的时候，使用箭头函数。 
eslint: [`prefer-arrow-callback`](http://eslint.org/docs/rules/prefer-arrow-callback), [`arrow-spacing`](http://eslint.org/docs/rules/arrow-spacing)
...
### [8.2](#arrows--implicit-return) 如果函数体由一个没有副作用的 [表达式](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Expressions_and_Operators#Expressions) 语句组成，删除大括号和 return。否则，使用大括号和  `return` 语句。 
eslint: [`arrow-parens`](https://eslint.org/docs/rules/arrow-parens), [`arrow-body-style`](https://eslint.org/docs/rules/arrow-body-style)
...
### [8.4](#arrows--one-arg-parens) 在箭头函数参数两头，总是使用小括号包裹住参数，这样做使代码更清晰且一致. 
...
### [8.5](#arrows--confusing) 避免箭头函数（`=>`）和比较操作符（`<=`, `>=`）混淆. 
...
### [8.6](#whitespace--implicit-arrow-linebreak) 使箭头函数体有一个清晰的返回。 
...
### [9.5](#constructors--no-useless) 如果没有特别定义，类有默认的构造方法。一个空的构造函数或只是代表父类的构造函数是不需要写的。 
...
### [9.6](#classes--no-duplicate-members) 避免重复定义类成员。
...
### [10.1](#modules--use-them) 使用（`import`/`export`）模块而不是非标准的模块系统。
...
### [10.7](#modules--imports-first) 把 `import` 放在其他所有语句之前。
...
### [11.1](#iterators--nope) 不要用迭代器。
eslint: [`no-iterator`](http://eslint.org/docs/rules/no-iterator) [`no-restricted-syntax`](http://eslint.org/docs/rules/no-restricted-syntax)
  > 使用 JavaScript 高级函数代替 `for-in`、 `for-of`。
...
### [11.3](#generators--spacing) 如果你一定要用生成器，或者你忽略 [我们的建议](#generators--nope)，请确保它们的函数标志空格是得当的。
...
### [12.1](#properties--dot) 访问属性时使用点符号。
eslint: [`dot-notation`](http://eslint.org/docs/rules/dot-notation)
...
### [12.3](#es2016-properties--exponentiation-operator) 做幂运算时用幂操作符 `**` 。
...
### [13.1](#variables--const)  使用 `const` 或 `let` 声明变量。不这样做会导致全局变量。我们想要避免污染全局命名空间。地球超人也这样警告我们（译者注：可能是一个冷笑话）。 
...
### [13.2](#variables--one-const) 为每个变量声明都用一个 `const` 或 `let `。
...
### [13.5](#variables--no-chain-assignment) 不要使用链式声明变量。 
...
### [13.6](#variables--unary-increment-decrement) 不要使用一元自增自减运算符（`++`， `--`）. 
...
### [13.7](#variables--linebreak) 在赋值的时候避免在 `=` 前/后换行。 
eslint [`operator-linebreak`](https://eslint.org/docs/rules/operator-linebreak).
  > 如果你的赋值语句超出 [`max-len`](https://eslint.org/docs/rules/max-len)，那就用小括号把这个值包起来再换行。
...
### [13.8](#variables--no-unused-vars) 不允许有未使用的变量。
...
### [15.1](#comparison--eqeqeq) 用 `===` 和 `!==` 而不是 `==` 和 `!=`. 
...
### [15.5](#comparison--switch-blocks) 在 `case` 和 `default` 分句里用大括号创建一块包含词法声明的区域（例如：`let`、`const`、`function` 和 `class`）。
eslint rules: [`no-case-declarations`](http://eslint.org/docs/rules/no-case-declarations).
...
eslint rules: [`no-nested-ternary`](http://eslint.org/docs/rules/no-nested-ternary)
...
eslint rules: [`no-unneeded-ternary`](http://eslint.org/docs/rules/no-unneeded-ternary)
...
### [15.8](#comparison--no-mixed-operators) 用圆括号来组合多种操作符。
eslint: [`no-mixed-operators`](https://eslint.org/docs/rules/no-mixed-operators)
  > 唯一里的例外就是像 `+`, `-`, 和 `**` 这种优先级容易理解的运算符。我们还是建议把 `/` `*` 放到小括号里， 因为他们混用的时候优先级容易有歧义。 
...
### [16.2](#blocks--cuddled-elses) `if` 表达式的 `else`和 `if` 的右大括号在一行。
...
### [16.3](#blocks--no-else-return) 如果 `if` 语句中总是需要用 `return` 返回，那后续的 `else` 就不需要写了。 `if` 块中包含 `return`， 它后面的 `else if` 块中也包含了 `return`， 这个时候就可以把 `return` 分到多个 `if` 语句块中。 
...
### [18.3](#comments--spaces) 所有注释开头空一格，方便阅读。
...
### [19.1](#whitespace--spaces) 一个缩进使用两个空格。
...
### [19.2](#whitespace--before-blocks) 在大括号前空一格。
...
### [19.3](#whitespace--around-keywords) 在控制语句（`if`, `while` 等）的圆括号前空一格。 
...
### [19.4](#whitespace--infix-ops) 用空格来隔开运算符。
...
### [19.5](#whitespace--newline-at-end) 文件结尾空一行。
...
### [19.6](#whitespace--chains) 当出现长的方法链式调用时（>2个）用缩进。用点开头强调该行是一个方法调用，而不是一个新的语句。
...
### [19.8](#whitespace--padded-blocks) 不要用空白行填充块。
...
### [19.9](#whitespace--no-multiple-blanks) 不要在代码之间使用多个空白行填充。
...
### [19.10](#whitespace--in-parens) 圆括号里不要加空格。
...
### [19.11](#whitespace--in-brackets) 方括号里不要加空格。 
eslint: [`array-bracket-spacing`](http://eslint.org/docs/rules/array-bracket-spacing)
...
### [19.12](#whitespace--in-braces) 花括号里加空格 。
...
eslint: [`max-len`](http://eslint.org/docs/rules/max-len)
  > 注意：对于 [上面](#strings--line-length)，长字符串不受此规则限制，不应换行。
...
### [19.14](#whitespace--block-spacing) 作为语句的花括号内也要加空格 —— `{` 后和 `}` 前都需要空格。 
...
### [19.15](#whitespace--comma-spacing) `,` 前不要空格， `,` 后需要空格。 
...
### [19.16](#whitespace--computed-property-spacing) 花括号跟属性间要有空格，中括号跟属性间没有空格。 
...
### [19.17](#whitespace--func-call-spacing) 调用函数时，函数名和小括号之间不要空格。 
...
### [19.18](#whitespace--key-spacing) 在对象的字面量属性中， `key`  和 `value` 之间要有空格。 
...
### [19.19](#whitespace--no-trailing-spaces) 行末不要空格。 
eslint: [`no-trailing-spaces`](https://eslint.org/docs/rules/no-trailing-spaces)

<a name="19.20"></a>
<a name="whitespace--no-multiple-empty-lines"></a>

### [19.20](#whitespace--no-multiple-empty-lines) 避免出现多个空行。 
eslint: [`no-multiple-empty-lines`](https://eslint.org/docs/rules/no-multiple-empty-lines)
> 在文件末尾只允许空一行。文件开始处不要出现空行。
...
### [20.1](#commas--leading-trailing) 不要前置逗号。
...
### [22.5](#coercion--bitwise) **注意:** 用移位运算要小心。
  > 数字是用 [64-位](https://es5.github.io/#x4.3.19)表示的，但移位运算常常返回的是32为整形[source](https://es5.github.io/#x11.7))。移位运算对大于 32 位的整数会导致意外行为。[Discussion](https://github.com/airbnb/javascript/issues/109). 最大的 32 位整数是 2,147,483,647
...
### [23.1](#naming--descriptive) 避免用一个字母命名，让你的命名有意义。
...
### [23.2](#naming--camelCase) 用小驼峰命名法来命名你的对象、函数、实例。
...
### [23.3](#naming--PascalCase) 用大驼峰命名法来命名类。
...
### [23.4](#naming--leading-underscore) 不要用前置或后置下划线。
...
### [25.1](#events--hash) 当传递数据载荷给事件时（不论是 DOM 还是像 Backbone 这样有很多属性的事件）。
> 这使得后续的贡献者（程序员）向这个事件添加更多的数据时不用去找或者更新每个处理器。
例如：
...
- 首选 [`mocha`](https://www.npmjs.com/package/mocha)。 [`tape`](https://www.npmjs.com/package/tape) 偶尔被用来测试一些小的、独立的模块。
